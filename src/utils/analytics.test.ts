...
// ─── GA4 dual-channel: suppressed when GTM is configured ─────────────────────

describe('GA4 direct tracking', () => {
  it('calls ReactGA.event when GTM is not configured and GA4 is initialised', () => {
    // Test env has no VITE_GTM_CONTAINER_ID → GTM_CONFIGURED = false → GA4 fires
    initGA4('G-TEST123')
    trackDocumentDropped('test.json', 'drop')
    expect(vi.mocked(ReactGA.event)).toHaveBeenCalledWith(
      'DOCUMENT_DROPPED',
      expect.any(Object)
    )
  })

  it('does NOT call ReactGA.event when GTM is configured', async () => {
    vi.resetModules()

    vi.doMock('react-ga4', () => ({
      default: { initialize: vi.fn(), event: vi.fn() },
    }))
    vi.doMock('@trustvc/trustvc', () => ({
    ...
    Object.defineProperty(import.meta, 'env', {
      value: { VITE_PLATFORM: 'test', VITE_GTM_CONTAINER_ID: 'GTM-XXXXX' },
      writable: true,
      configurable: true,
    })

    const { initGA4: reInitGA4, trackDocumentDropped: reTrack } =
      await import('./analytics')
    const { default: reReactGA } = await import('react-ga4')

    window.dataLayer = []
    reInitGA4('G-TEST123')
    reTrack('test.json', 'drop')

    expect(vi.mocked(reReactGA.event)).not.toHaveBeenCalled()
    expect(window.dataLayer[0].event).toBe('DOCUMENT_DROPPED')
