 ...
 */
export const getChainInfoFromNetworkName = (networkName: string): chainInfo => {
  const res = Object.keys(ChainInfo)
    .map(chainId => ChainInfo[chainId as CHAIN_ID])
...
export const getUnsupportedChainIds = (): CHAIN_ID[] => {
 ...
 */
export const getSupportedChainInfo = (): chainInfo[] => {
...
export const isSupportedNetwork = (
  ...
  return (networks ?? getSupportedChainInfo()).some(
    chainInfoObj => chainInfoObj.id.toString() === chainId.toString()
  )
}
